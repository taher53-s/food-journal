...
import { FadeIn, SectionHeading } from "@/components/animations/PageTransition";
  ...
  return (
    <div className="min-h-screen pt-24 pb-20 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <FadeIn delay={0.1}>
          <div className="sticky top-20 z-30 backdrop-blur-2xl py-4 -mx-4 px-4 mb-8 bg-[#FAF7F0]/90">
            <div className="flex gap-3">
              <div className="relative flex-1">
                ...
                <input type="text" placeholder="Search restaurants, cuisines, locations..." value={search} onChange={(e) => setSearch(e.target.value)}
                  className="w-full pl-11 pr-11 py-3.5 bg-white rounded-2xl border border-forest-100 text-sm font-medium text-forest-800 placeholder:text-forest-400 focus:outline-none focus:ring-2 focus:ring-forest-400/30 focus:border-forest-300 transition-all min-h-[48px]" />
                {search && (
                  <button onClick={() => setSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 p-1.5 rounded-lg text-forest-400 hover:text-forest-600 hover:bg-forest-50 transition-colors min-w-[36px] min-h-[36px] flex items-center justify-center">
              ...
              <button onClick={() => setShowFilters(!showFilters)}
                className={cn("flex items-center gap-2 px-4 py-3.5 rounded-2xl border text-sm font-semibold transition-all min-h-[48px]",
                  showFilters ? "bg-forest-600 text-white border-forest-600 shadow-md" : "bg-white text-forest-700 border-forest-200 hover:border-forest-400 hover:shadow-sm")}>
                ...
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.25 }} className="overflow-hidden">
                  <div className="pt-4 grid grid-cols-1 sm:grid-cols-3 gap-4">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {CUISINES.map((c) => (
                          <button key={c} onClick={() => setCuisine(c)}
                            className={cn("px-3 py-2 rounded-full text-xs font-medium transition-all min-h-[40px]", cuisine === c ? "bg-forest-600 text-white shadow-sm" : "bg-white border border-forest-200 text-forest-600 hover:border-forest-400 hover:bg-forest-50")}>
                            {c}
                          </button>
                        ))}
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {PRICE_RANGES.map((p) => (
                          <button key={p.value} onClick={() => setPriceRange(p.value)}
                            className={cn("px-3 py-2 rounded-full text-xs font-medium transition-all min-h-[40px]", priceRange === p.value ? "bg-gold-500 text-forest-950 shadow-sm" : "bg-white border border-forest-200 text-forest-600 hover:border-gold-400 hover:bg-gold-50/30")}>
                            {p.label}
                          </button>
                        ))}
                      </div>
                    ...
                    <div>
                      ...
                      <input type="range" min="0" max="10" step="0.5" value={minRating} onChange={(e) => setMinRating(parseFloat(e.target.value))} className="w-full accent-forest-600 min-h-[40px] mt-1" />
                    </div>
                  </div>
                </motion.div>
              )}
