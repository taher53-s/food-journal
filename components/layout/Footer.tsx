...
export function Footer() {
  ...
  return (
    <>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-10 md:gap-8">
            <div className="md:col-span-1">
              <Link href="/" className="inline-flex items-center gap-2.5 group mb-4">
                <div className="w-9 h-9 bg-gradient-to-br from-gold-400 to-gold-600 rounded-xl flex items-center justify-center shadow-[0_4px_12px_rgba(245,158,11,0.3)]">
              ...
              <div className="flex flex-col gap-2.5">
                {[
                  ...
                  <Link key={l.href} href={l.href}
                    className="group flex items-center gap-2 text-sm text-white/40 hover:text-white transition-all duration-200 py-0.5">
                    <span className="w-1 h-1 rounded-full bg-white/20 group-hover:bg-gold-400 transition-colors duration-200" />
                    <span>{l.label}</span>
                  </Link>
                ))}
              </div>
          ...
          <div className="mt-12 pt-8 border-t border-white/[0.06]">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <p className="text-white/20 text-xs">© {new Date().getFullYear()} FoodLog. All memories reserved.</p>
              <div className="flex items-center gap-4">
                <span className="text-[10px] uppercase tracking-widest text-white/20">Powered by</span>
      ...
      {/* Back to top button — dark glass variant */}
      <motion.button
        initial={{ opacity: 0, scale: 0.8, y: 20 }}
        animate={{ opacity: showTop ? 1 : 0, scale: showTop ? 1 : 0.8, y: showTop ? 0 : 20 }}
        transition={{ duration: 0.3 }}
        onClick={scrollToTop}
        className="fixed bottom-6 right-6 z-40 w-12 h-12 rounded-full bg-white/10 backdrop-blur-xl border border-white/20 shadow-[0_4px_20px_rgba(0,0,0,0.3)] flex items-center justify-center text-white/50 hover:text-white hover:bg-white/20 active:scale-95 transition-all duration-200"
