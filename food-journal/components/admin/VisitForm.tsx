...
export function VisitForm({ initialData, visitId }: { initialData?: any; visitId?: string }) {
  ...
  const handleRestaurantPhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      setRestaurantPhotos([...restaurantPhotos, ...Array.from(e.target.files)]);
    }
  };

  const removeRestaurantPhoto = (index: number) => {
    setRestaurantPhotos(restaurantPhotos.filter((_, i) => i !== index));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const dish of dishes) {
        ...
        let image_url = null;
        if (dish.image) {
          const path = `${visitResult.id}/dish-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.jpg`;
          image_url = await uploadImage(dish.image, path);
        }
      ...
      for (let i = 0; i < restaurantPhotos.length; i++) {
        const photo = restaurantPhotos[i];
        const path = `${visitResult.id}/restaurant-${Date.now()}-${i}-${Math.random().toString(36).substr(2, 9)}.jpg`;
        const image_url = await uploadImage(photo, path);
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className={sectionClass}>
        <div className="flex items-center justify-between">
          <h2 className="font-display text-xl font-semibold text-forest-900">Restaurant Photos</h2>
        </div>
        <p className="text-sm text-forest-500 mb-4">Add photos of the restaurant interior, exterior, or menu (not dishes)</p>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
          {restaurantPhotos.map((photo, i) => (
            <div key={i} className="relative aspect-square rounded-xl overflow-hidden bg-forest-100">
              <img 
              ...
              <button
                type="button"
                onClick={() => removeRestaurantPhoto(i)}
          ...
          <label className="aspect-square rounded-xl border-2 border-dashed border-forest-200 flex flex-col items-center justify-center cursor-pointer hover:border-forest-400 transition-colors">
            <Upload className="w-5 h-5 text-forest-400 mb-1" />
            <span className="text-xs text-forest-500">Add Photo</span>
            <input 
              type="file" 
